...
type AddProjectTestCaseTemplateParams struct {
	ID               int32
	TestcaseTemplate sql.NullString
}

func (q *Queries) AddProjectTestCaseTemplate(ctx context.Context, arg AddProjectTestCaseTemplateParams) error {
	_, err := q.db.ExecContext(ctx, addProjectTestCaseTemplate, arg.ID, arg.TestcaseTemplate)
	return err
}
...
const convertCommentToTestCase = `-- name: ConvertCommentToTestCase :one
...
type ConvertCommentToTestCaseParams struct {
	ID        uuid.UUID
	NewTestID uuid.UUID
}

func (q *Queries) ConvertCommentToTestCase(ctx context.Context, arg ConvertCommentToTestCaseParams) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, convertCommentToTestCase, arg.ID, arg.NewTestID)
	var id uuid.UUID
	err := row.Scan(&id)
	return id, err
}
...
func (q *Queries) CreateComment(ctx context.Context, arg CreateCommentParams) (TestPlanComment, error) {
...
RETURNING id, project_id, uploader_id, name, file_path, file_size, mime_type, created_at, updated_at
`
...
func (q *Queries) CreateProjectDocument(ctx context.Context, arg CreateProjectDocumentParams) (ProjectDocument, error) {
	row := q.db.QueryRowContext(ctx, createProjectDocument,
		arg.ID,
		arg.ProjectID,
		arg.UploaderID,
		arg.Name,
...
const deleteProjectDocument = `-- name: DeleteProjectDocument :execrows
DELETE FROM project_documents WHERE id = $1
`

func (q *Queries) DeleteProjectDocument(ctx context.Context, id uuid.UUID) (int64, error) {
	result, err := q.db.ExecContext(ctx, deleteProjectDocument, id)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
...
func (q *Queries) FindAllSuggestedByProject(ctx context.Context, arg FindAllSuggestedByProjectParams) ([]TestCase, error) {
	rows, err := q.db.QueryContext(ctx, findAllSuggestedByProject, arg.ProjectID, arg.Suggested)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getProjectTestCaseTemplate = `-- name: GetProjectTestCaseTemplate :one
SELECT testcase_template FROM projects WHERE id = $1
`

func (q *Queries) GetProjectTestCaseTemplate(ctx context.Context, id int32) (sql.NullString, error) {
	row := q.db.QueryRowContext(ctx, getProjectTestCaseTemplate, id)
	var testcase_template sql.NullString
	err := row.Scan(&testcase_template)
	return testcase_template, err
}
...
const listCommentsByTestPlan = `-- name: ListCommentsByTestPlan :many
SELECT 
    c.id,
    c.test_plan_id,
    c.user_id,
...
type ListCommentsByTestPlanRow struct {
	ID          uuid.UUID
	TestPlanID  int64
	UserID      int64
	DisplayName sql.NullString
...
func (q *Queries) ListCommentsByTestPlan(ctx context.Context, testPlanID int64) ([]ListCommentsByTestPlanRow, error) {
	rows, err := q.db.QueryContext(ctx, listCommentsByTestPlan, testPlanID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListCommentsByTestPlanRow
	for rows.Next() {
		var i ListCommentsByTestPlanRow
		if err := rows.Scan(
			&i.ID,
			&i.TestPlanID,
			&i.UserID,
...
func (q *Queries) ListDocumentsByProject(ctx context.Context, projectID int64) ([]ProjectDocument, error) {
	rows, err := q.db.QueryContext(ctx, listDocumentsByProject, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListTestRunsAssignedToUser(ctx context.Context, assignedToID sql.NullInt32) ([]TestRun, error) {
...
FROM (
  ...
  WHERE pc.assigned_to_id = $1
  GROUP BY tc.id
  HAVING $2::bool = true OR COALESCE(BOOL_OR(tr.is_closed), false)::boolean = false
) sub
`
...
type UpdateSuggestedFlagParams struct {
	ID        uuid.UUID
	Suggested sql.NullBool
}

func (q *Queries) UpdateSuggestedFlag(ctx context.Context, arg UpdateSuggestedFlagParams) error {
	_, err := q.db.ExecContext(ctx, updateSuggestedFlag, arg.ID, arg.Suggested)
	return err
}
